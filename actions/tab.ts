...
export async function updateTab(id: string, name: string) {
  try {
    const tab = await db.update(tabs).set({
      name,
    }).where(eq(tabs.id, id)).returning();
    revalidatePath("/superadmin/task");
    return tab;
  } catch (error) {
...
export async function deleteTab(id: string, options?: { moveTasksToTabId?: string; deleteTasks?: boolean }) {
  try {
    // 関連するタスクの処理
